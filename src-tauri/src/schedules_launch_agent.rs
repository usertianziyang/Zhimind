...
//! installs a user LaunchAgent that starts the **full Zhimind App** at login and
...
"#
    ...
    format!(
        r#"#!/bin/bash
# Generated by Zhimind — schedules keep-alive helper.
# NOT a headless daemon. Starts the full Zhimind App so automation_runner can tick
...
"#
    ...
    format!(
        r#"#!/bin/bash
# Generated by Zhimind — one-shot schedule fire helper (AUTO-HEADLESS A2).
# NOT a KeepAlive daemon and NOT a continuous scheduler.
# Boots Zhimind with {flag}: fires at most one due scheduled task, then exits.
...
<!-- Generated by Zhimind. Label: {label}
...
fn limits_text(app_path: &Path, helper: &Path) -> String {
    format!(
        "Zhimind — schedules LaunchAgent + one-shot helpers\n\
         ...
         Label: {label}\n\
         \n\
         Disable full-app LaunchAgent from Zhimind → Scheduled tasks, or:\n\
